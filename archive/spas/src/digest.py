"""Digest algorithms for SPAS v0.1.0.

WISE-DIGEST-0 — native, experimental, deterministic.
SHA-256        — production fallback via hashlib.

Both expose the same streaming interface (update(bytes), hexdigest()).
"""
...
class WiseDigest0:
    """WISE-DIGEST-0 streaming hasher.

    Per SPAS §7 + §30.1 + §30.2 + §30.3 + §30.4:
      - sequential state mutation within a per-byte round
      - sequential state mutation within a finalization round
      - 64-bit big-endian bit-length appended before finalization
      - 16 finalization rounds
      - 256-bit output as 64 lowercase hex chars
    """
